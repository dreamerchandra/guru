...
export default function chapterApi (http, baseUrl, responseWrapper) {
  return {
    ...
    upsert: async ({ fields, chapterId }) => {
      const oldImgUrl = typeof (fields.titleImg) === 'string' ? fields.titleImg : '';
      const data = addMetaToPayload(sanitizeData({ ...fields, titleImg: oldImgUrl, batches: { [getCurrentUser()]: true } }))
      const chapter = !chapterId ? { ...data, createdAt: getServerTimeStamp() } : data;
      ...
      const docRef = chapterId ? ref().chapter.doc(chapterId) : ref().chapter.doc();
      const { fullPath = oldImgUrl, task } = fields.imgUrl ? await uploadFile(docRef.id, fields.titleImg?.files?.[0]) : { task: Promise.resolve() };
      ...
      return Promise.all([
        docRef.set({ ...chapter, titleImg: fullPath }),
        uploadPromise
      ]);
    },
